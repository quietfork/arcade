...
|---|---|
| 文書名 | Arcade 実装計画書 |
| 対象プロダクト | Arcade (Claude Code マルチペインランチャー) |
| バージョン | 1.0 |
| 作成日 | 2026-05-06 |
| 作成者 | quietfork |
| 関連文書 | [arcade_requirements.md](arcade_requirements.md) |
...
### 1.1 本書の位置づけ
要件定義書(`arcade_requirements.md`)が **WHAT(何を作るか)** を定義する文書であるのに対し、本書は **HOW / WHEN(どう進めるか・進捗はどこか)** を管理する。
...
| パス | 役割 |
|---|---|
| [arcade/main.go](arcade/main.go) | Wails エントリポイント。各サービスを bind。 |
| [arcade/app.go](arcade/app.go) | Wails テンプレート由来の App 構造体(現状ほぼ未使用) |
| [arcade/pty_manager.go](arcade/pty_manager.go) | PTY セッション管理(start / write / resize / close / shutdown)。データ送信は `pty:data:<id>` / `pty:exit:<id>` イベント。 |
| [arcade/clipboard_service.go](arcade/clipboard_service.go) | ペースト画像を `~/.arcade/clipboard/` に保存。 |
| [arcade/frontend/src/App.tsx](arcade/frontend/src/App.tsx) | 現状は単一ペイン PoC。Phase 1 で大幅再構成予定。 |
| [arcade/frontend/src/App.css](arcade/frontend/src/App.css) | アプリ全体のレイアウト CSS。 |
| [arcade/frontend/src/style.css](arcade/frontend/src/style.css) | グローバル(html/body)CSS。Wails テンプレートの中央寄せは除去済。 |

### 3.2 今後追加予定のファイル(Phase 1 以降)

| パス | 役割 | 追加フェーズ |
|---|---|---|
| `arcade/project_store.go` | プロジェクトカタログの永続化(`projects.json`) | Phase 1 |
| `arcade/layout_store.go` | レイアウトの永続化(`layouts.json`) | Phase 2 |
| `arcade/settings_store.go` | 設定の永続化(`settings.json`) | Phase 2 |
| `arcade/frontend/src/components/Sidebar.tsx` | プロジェクト一覧サイドバー | Phase 1 |
| `arcade/frontend/src/components/PaneHost.tsx` | グリッド内 1 ペインのホスト(ヘッダ + Terminal) | Phase 1 |
| `arcade/frontend/src/components/Terminal.tsx` | 現在の `App.tsx` 内ロジックを切り出した xterm ラッパ | Phase 1 |
| `arcade/frontend/src/components/ProjectDialog.tsx` | プロジェクト追加 / 編集ダイアログ | Phase 1 |
| `arcade/frontend/src/components/SettingsDialog.tsx` | 設定画面 | Phase 2 |

---

## 4. Phase 0: 環境構築 + PoC ✅ 完了

PoC として下記が実装済。`wails dev` または `arcade.exe` 直接起動で動作確認可能。
...
- [x] **FR-101 プロジェクト登録**
  - 実装: `project_store.go` で JSON 永続化(`~/.arcade/projects.json`、アトミック書き込み、0600)、`PickDirectory` で `OpenDirectoryDialog` をラップ。フロントは `ProjectDialog.tsx` に Browse ボタン + 手入力 + 名前自動補完。
...
プロダクト名候補: **Multiplex**(プロトタイプ名。要件定義書では `Arcade` のまま運用するか、改名するか別途相談)
...
| パス | 種別 | 内容 |
|---|---|---|
| `arcade/settings_store.go` | 修正 | `SidebarVisible` / `ActiveView` フィールド追加。後方互換性は zero-value で吸収。 |
| `arcade/frontend/src/components/ActivityBar.tsx` | 新規 | Activity Bar 本体 |
| `arcade/frontend/src/components/ProjectsView.tsx` | 新規 | Projects ビュー(現 Sidebar 上半分の切り出し) |
| `arcade/frontend/src/components/PanesView.tsx` | 新規 | Panes ビュー(ツリー)コンテナ |
| `arcade/frontend/src/components/PaneTreeNode.tsx` | 新規 | 再帰的ツリーノード |
| `arcade/frontend/src/components/Sidebar.tsx` | 削除 or 薄いラッパに | 役割を ProjectsView / PanesView に分割 |
| `arcade/frontend/src/components/Icons.tsx` | 修正 | `Folder` / `Layers` / `ChevronRight` / `ChevronDown` 追加 |
| `arcade/frontend/src/components/Terminal.tsx` | 修正 | 外部から focus を呼べるよう ref 公開(`forwardRef` + `useImperativeHandle`) |
| `arcade/frontend/src/App.tsx` | 修正 | `sidebarVisible` / `activeView` 状態管理、Ctrl+B ハンドラ、レイアウト変更、トースト発火 |
| `arcade/frontend/src/App.css` | 修正 | `.activity-bar` / `.side-bar` / `.activity-icon` / `.pane-tree-*` / `.context-menu` / `.toast` スタイル追加 |
| `arcade/frontend/src/components/ContextMenu.tsx` | 新規 | 汎用コンテキストメニュー(項目配列を受け取って描画) |
| `arcade/frontend/src/components/Toast.tsx` | 新規 | 軽量トースト通知 |
| `arcade/app.go` | 修正 | `RevealInExplorer(path string) error` を追加(任意) |
| `arcade/main.go` | 修正 | 上記メソッドの bind |
| `arcade/frontend/src/components/PaneHost.tsx` | 修正 | cwd 表示に `onContextMenu` |
...
| パス | 種別 | 内容 |
|---|---|---|
| `arcade/file_browser.go` | 新規 | `ListDir(path) []FileEntry` |
| `arcade/main.go` | 修正 | `FileBrowser` を bind |
| `arcade/settings_store.go` | 修正 | ActiveView の許容値変更 + migration |
...
## 8.7 Phase 6: マルチウィンドウ対応(slot 方式)

> **作業ブランチ**: `feature/multi-window`(2026-05-07 切り出し、`main` から分岐)

### 8.7.1 ゴール
2 つの arcade インスタンスを別モニタで起動して、それぞれ**独立したレイアウト**で動き、**プロジェクトカタログ・ユーザー設定は自動同期**される状態を実現する。

### 8.7.2 設計方針: VSCode 風 single-writer モデル

VSCode は Electron で「**1 プロセス + 複数 renderer**」で実現しており、main process が状態の単一所有者となることで競合を構造的に排除している。本アプリは Wails v2 制約のため「1 プロセス N ウィンドウ」は不可だが、**N プロセス間で「main slot のみ書き込み権限を持つ」というルールを敷く**ことで等価な堅牢性を得る。

```
┌─── ~/.arcade/ ──────────────────────────┐
│  projects.json          ← 全 slot で共有     │  ← main slot のみが書き込み
│  settings.json          ← user-level 共有    │     他 slot は read-only
│                            (theme/font/consent/...)
│  slots/
│    main/
│      layout.json        ← slot 固有レイアウト
│      slot-settings.json ← slot 固有 (sidebarHidden, activeView)
│      lock               ← PID + start time + exe name
│    second/
│      layout.json
│      slot-settings.json
│      lock
...
| 状態 | 共有 / 固有 | 書き込み権限 |
|---|---|---|
| プロジェクト一覧 | 共有 | **main slot のみ** |
| テーマ・フォント・consent・default command | 共有 | **main slot のみ** |
| レイアウト(ペイン構成) | slot ごと | 自 slot のみ |
| サイドバー表示・active view | slot ごと | 自 slot のみ |
| ペイン PTY セッション | プロセス独立 | 自 slot のみ |

**フェイルセーフ**: main slot が落ちた場合、他 slot のうちロック奪取に成功した slot が自動的に main に昇格(writer 権限を獲得)。
...
- [x] **FR-NEW-14 LayoutStore の slot 対応**
  - 保存先: `~/.arcade/slots/<slot>/layout.json`
  - 名前付きレイアウト(FR-403)も同 slot 配下に
  - **マイグレーション**: 既存 `~/.arcade/layouts.json` → `slots/main/layout.json`(初回起動時、global lock で保護)
  - **工数**: 0.3 日

- [x] **FR-NEW-15 Settings の user-level / slot-level 分離**
  - `~/.arcade/settings.json`(共有): `theme` / `fontFamily` / `fontSize` / `lineHeight` / `scrollback` / `defaultCommand` / `defaultArgs` / `dangerousConsent`
  - `~/.arcade/slots/<slot>/slot-settings.json`(slot 固有): `sidebarHidden` / `activeView`
...
- [x] **FR-NEW-16′ Lock + writer 権限管理**(統合タスク)
  ...
  - 起動時に既存 lock の生存確認:
    ...
    - 不一致 or PID 死んでる → ゾンビとみなして奪取
    - mtime > 24h なら無条件で奪取(TTL フェイルセーフ)
  - 書き込み権限ロジック:
    - main slot の lock を持つプロセス = writer
    - 他 slot は reader(projects/user-settings は read-only)
...
- [x] **FR-NEW-19 「新しいウィンドウを開く」ボタン**(detached 子プロセス)
  ...
  - OS 別 detach:
    - Windows: `SysProcAttr.CreationFlags = DETACHED_PROCESS \| CREATE_NEW_PROCESS_GROUP`
    - macOS: `open -n -a <App.app> --args --slot=<name>`
    - Linux: `Setpgid: true`
  - 既存 slot 名を入力した場合は lock 衝突で子プロセスが起動拒否 → 親側にトーストで通知
...
| パス | 種別 | 内容 |
|---|---|---|
| `arcade/main.go` | 修正 | flag 解析、slot 名を各 store に渡す、lock 取得、shutdown で lock 解放 |
| `arcade/slot.go` | 新規 | slot ID + lock + writer 権限管理 |
| `arcade/layout_store.go` | 修正 | 保存先パスに slot を組み込み + マイグレーション |
| `arcade/settings_store.go` | 修正 | user-level / slot-level 2 ファイルに分離 |
| `arcade/project_store.go` | 修正 | fsnotify watcher + `projects:changed` イベント、reader slot からの書き込み拒否 |
| `arcade/app.go` | 修正 | `GetSlot()` / `GetSlotRole()` / `OpenNewWindow(slot)` を追加 |
| `arcade/go.mod` | 修正 | `github.com/fsnotify/fsnotify` 追加 |
| `arcade/frontend/src/components/TitleBar.tsx` | 修正 | slot 名 + role 表示、新規ウィンドウボタン |
| `arcade/frontend/src/components/NewWindowDialog.tsx` | 新規 | slot 名入力モーダル |
| `arcade/frontend/src/App.tsx` | 修正 | 起動時に slot/role 取得、`projects:changed` / `settings:changed` 購読、reader 時の UI 無効化 |

### 8.7.5 実装順(コミット粒度)

各タスク完了時にコミットする想定:

1. `feat(slot): add CLI flag with validation` (FR-NEW-13)
2. `feat(slot): per-slot layout storage with migration` (FR-NEW-14)
3. `feat(settings): split user-level vs slot-level` (FR-NEW-15)
4. `feat(slot): lock file with PID + start time + writer role` (FR-NEW-16′)
5. `feat(sync): fsnotify watcher with content-hash dedupe + polling fallback` (FR-NEW-17)
6. `feat(ui): TitleBar slot indicator + reader-mode disable` (FR-NEW-18)
7. `feat(ui): new window button with OS-specific detached spawn` (FR-NEW-19)
8. `feat(slot): auto-promote reader to writer on main exit` (FR-NEW-20)

途中で発覚した不具合・リファクタは小コミットで挟む。

### 8.7.6 リスクと対策

| # | リスク | 対策 |
|---|---|---|
| R1 | PID 再利用による lock 誤判定 | lock に PID + start time + exe path の 3 点セット、TTL 24h フォールバック(FR-NEW-16′) |
| R2 | 同時書き込みロスト | **構造的に排除**: main slot のみ writer(VSCode パターン) |
| R3 | fsnotify echo ループ | content-hash dedupe + Rename イベントのみ拾う(FR-NEW-17) |
| R4 | detached 子プロセス OS 差 | OS 別実装(FR-NEW-19) |
| R5 | slot 名予約語衝突 | バリデーション + ブラックリスト(FR-NEW-13) |
| R6 | マイグレーション race | `~/.arcade/migration.lock` の global lock(FR-NEW-14) |
| R7 | ProjectDialog 開いている最中の自動 refresh | ダイアログ open 中は refresh をキューに溜めて close 時に適用 |
| R8 | fsnotify on OneDrive / VM 共有 FS | 30 秒ポーリングフォールバック(FR-NEW-17) |
| R9 | クラッシュで lock 残留 → 永久ブロック | TTL 24h 自動解除(FR-NEW-16′) |
| R10 | settings.json 初回作成 race | `O_CREATE\|O_EXCL` で作成 |

### 8.7.7 工数見積

| 区分 | 工数 |
|---|---|
| Backend (FR-NEW-13〜17, 20) | 1.25 日 |
| Frontend (FR-NEW-18, 19) | 0.35 日 |
| 統合テスト・デバッグ | 0.3 日 |
| **合計** | **約 1.9 日** |

(当初見積 1.5 日からリスク対応分で +0.4 日。R1/R3/R4/R8 への具体策をタスクに織り込んだため。)

### 8.7.8 Phase 6 で扱わないこと
- 真の in-process マルチウィンドウ → Wails v3 alpha の安定化待ち。Phase 8 以降で再検討
- ペイン単位の「他ウィンドウへ移動」(D&D)→ Phase 7 以降
- daemon プロセスモデル(VSCode により近い設計)→ 大規模リライトのため見送り
