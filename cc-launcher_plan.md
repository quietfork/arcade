...
### 8.7.1 ゴール
2 つの cc-launcher インスタンスを別モニタで起動して、それぞれ**独立したレイアウト**で動き、**プロジェクトカタログ・ユーザー設定は自動同期**される状態を実現する。
...
| パス | 種別 | 内容 |
|---|---|---|
| `cc-launcher/main.go` | 修正 | flag 解析、slot 名を各 store に渡す、lock 取得、shutdown で lock 解放 |
| `cc-launcher/slot.go` | 新規 | slot ID + lock + writer 権限管理 |
| `cc-launcher/layout_store.go` | 修正 | 保存先パスに slot を組み込み + マイグレーション |
| `cc-launcher/settings_store.go` | 修正 | user-level / slot-level 2 ファイルに分離 |
| `cc-launcher/project_store.go` | 修正 | fsnotify watcher + `projects:changed` イベント、reader slot からの書き込み拒否 |
| `cc-launcher/app.go` | 修正 | `GetSlot()` / `GetSlotRole()` / `OpenNewWindow(slot)` を追加 |
| `cc-launcher/go.mod` | 修正 | `github.com/fsnotify/fsnotify` 追加 |
| `cc-launcher/frontend/src/components/TitleBar.tsx` | 修正 | slot 名 + role 表示、新規ウィンドウボタン |
| `cc-launcher/frontend/src/components/NewWindowDialog.tsx` | 新規 | slot 名入力モーダル |
| `cc-launcher/frontend/src/App.tsx` | 修正 | 起動時に slot/role 取得、`projects:changed` / `settings:changed` 購読、reader 時の UI 無効化 |
...
| R6 | マイグレーション race | `~/.cc-launcher/migration.lock` の global lock(FR-NEW-14) |
